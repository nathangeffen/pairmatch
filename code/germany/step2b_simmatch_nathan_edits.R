...
repeat{
  ...
  if (pop[i,3] == 0 & pop[i,4] == 0){pop[i,7] <- sample(c(12:100), 1, replace = T, mat.mw[,pop[i,2]-11])}
  if (pop[i,3] == 1 & pop[i,4] == 0){pop[i,7] <- sample(c(12:100), 1, replace = T, mat.wm[,pop[i,2]-11])}
...
## dtm.het$mage <- NA
## dtm.het[which(dtm.het$sex == 0),"mage"] <- dtm.het[which(dtm.het$sex == 0),"page"]
## dtm.het[which(dtm.het$sex == 1),"mage"] <- dtm.het[which(dtm.het$sex == 1),"age"]
...
## head(dtm.het[c(result.het$index.treated),])
...
## write.csv(pop, paste(path,'Dropbox/1_Projekt_Syphilis/2_ABM_model/6_start_pop/pop.match.csv', sep=""))
...
## ggsave(paste(path, "Dropbox/1_Projekt_Syphilis/2_ABM_model/6_start_pop/AGEPYR_match.pdf", sep=""), device="pdf", height=5.75, width=4, units='in', scale=2)
